clear;
% img=load('./img_512x512x50.mat');
% img=double(img.ff);
% img=img(:,:,1:25);

% img=readNPY('phantom.npy');

doc='/home/wangwei/sdb/AMP/L096/full_1mm_sharp/';
file=dir(doc);
img=zeros(512,512,25);
start=100;
for i=1:25
 tmp=double(dicomread([doc,file(start+i).name]));
%  tmp=tmp/max(tmp(:));
 img(:,:,i)=tmp;
end
...
% sin=gather(permute(pf,[2,1,3]));
data=sin.*permute(parker,[2,1,3]);
% data=sin;
...
rf1=astra_mex_data3d('get', rec_id);


pf=gpuArray(permute(sin,[2,1,3]));
